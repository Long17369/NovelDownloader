...
class Main:
    ...
    def load_progress(self):
        ...
        try:
            ...
            with open("./txt/progress_file.json", "r", encoding="utf-8") as f:
                # 从JSON文件中加载进度信息
                progress: Dict[str, Dict[Union[str, int], Union[str, int]]] = json.load(
                    f
                )[self.info["name"]]
                # 更新信息字典中的进度信息
    ...
    def get_page(self, url, chapter: int, page: int, error: int = 0) -> bool:
        ...
        try:
            if self.use_proxy:
                respond = requests.get(url, allow_redirects=False, proxies=self.proxies)
            else:
    ...
    def extract_texts_from_html(self, html_string):
        ...
        try:
            ...
            texts = [
                h for i in script_tag for j, h in enumerate(str(i).split("'")) if j == 1
            ]
    ...
    def decode(self, chapter: int = -1, code: str = "") -> None:
        ...
        # 当章节参数不为默认值且code为默认值空时，解码指定章节
            ...
            self.decoded[chapter] = "\n".join(decoded_parts).replace("<p>—</p>", "\n")
            # 删除已解码的code
    ...
    def write(self, chapter: int, chapter_name):
        ...
        with open(
            f'./txt/Cache/{self.info["title"]}/{chapter}', "w", encoding="utf-8"
        ) as f:
            # 写入章节标题
    ...
    def write_all(self):
        ...
        chapters = [
            int(i) for i in self.info["progress"]["Chapter_size"] if isinstance(i, str)
        ]
        chapters.sort()
        # 初始化将要保存的小说文件
        ...
        for i in chapters:
            # 以只读模式打开当前章节文件
            with open(
                f'./txt/Cache/{self.info["title"]}/{i}', "r", encoding="utf-8"
            ) as f:
                # 以写入模式打开目标文件
                with open(
                    f'./txt/{self.info["title"]}.txt', "a", encoding="utf-8"
                ) as f1:
                    # 将当前章节内容写入目标文件
                    f1.write(f.read())
                    # 在每个章节之间添加空行，以区分不同章节
        ...
        for part in url.split("/"):
            for segment in part.split("_"):
                try:
                    ...
                    continue
        raise ValueError("无法从URL中提取章节编号")
    ...
    def get_chapter_info(self, book_id: int, error: int = 0):
        ...
        for i in range(len(page_numbers) - 1):
            ...
            # 分割HTML内容，提取章节链接和标题
            for j in (
                page_html.split("章节列表")[1].split("page_num")[1].split("href")[4:]
            ):
                temp = j.split('"')
                href_dict[len(href_dict)] = [
                    temp[1],
                    temp[2].split(">")[1].split("<")[0],
                ]
